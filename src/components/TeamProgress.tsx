  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-card overflow-hidden">
        <div className="grid grid-cols-[1fr_auto_auto] gap-4 px-5 py-3 border-b border-border bg-muted/30 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          <span>Member</span>
        ...
        {members.map((member) => {
          ...
          return (
            <div key={member.id} className="grid grid-cols-[1fr_auto_auto] gap-4 items-center px-5 py-4 border-b border-border last:border-0 hover:bg-muted/10 transition-colors">
              <div>
              ...
              <div className="w-20 text-right">
                <span className="inline-flex items-center gap-1 text-sm font-semibold text-foreground">
                  {completed === totalCards && <CheckCircle2 className="w-4 h-4" style={{ color: "hsl(160, 84%, 39%)" }} />}
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        ...
        <div className="space-y-2">
          {trainingCards.map((card) => {
            const completedCount = members.filter((m) => m.completedCards.includes(card.id)).length;
            const pct = Math.round((completedCount / members.length) * 100);
            return (
              <div key={card.id} className="flex items-center gap-3">
                <span className="text-xs text-muted-foreground w-6 text-right shrink-0">{card.number}.</span>
                <span className="text-sm text-foreground flex-1">{card.title}</span>
                <Progress value={pct} className="h-1.5 w-20" />
                <span className="text-xs text-muted-foreground w-16 text-right">{completedCount}/{members.length}</span>
              </div>
            );
          })}
        </div>
