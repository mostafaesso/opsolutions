  ...
  "crm-overview": {
    ...
    steps: [
      ...
      },
      {
        ...
        description:
          "The CRM dashboard gives you a bird's-eye view of your pipeline, recent activities, and key metrics. Learn to customize it for your workflow.",
        doList: [
  ...
  pipeline: {
    ...
    steps: [
      ...
      },
      {
        ...
        description:
          "Use pipeline data to forecast revenue and plan your quarter. Accuracy depends on consistent deal management.",
        doList: [
    ...
    steps: [
      {
        ...
        description:
          "HubSpot provides built-in reports for pipeline, activity, and revenue metrics. Learn which reports matter most.",
        doList: [
...
const TrainingDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <div style={{ background: "rgb(18, 18, 18)" }} className="py-12 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl">{topic.overview}</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 space-y-10">
        {topic.steps.map((step, i) => (
          <div key={i} className="rounded-2xl border border-border bg-card overflow-hidden">
            ...
            <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border/50">
              {/* Do */}
              <div className="p-6">
                <h4 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wider mb-4" style={{ color: "hsl(142, 71%, 45%)" }}>
                ...
                <ul className="space-y-3">
                  {step.doList.map((item, j) => (
                    <li key={j} className="flex items-start gap-3 text-sm text-foreground">
                      <CheckCircle2 className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "hsl(142, 71%, 45%)" }} />
                      {item}
                    </li>
                  ))}
                </ul>
              ...
              {/* Don't */}
              <div className="p-6">
                <h4 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wider mb-4" style={{ color: "hsl(0, 84%, 60%)" }}>
                ...
                <ul className="space-y-3">
                  {step.dontList.map((item, j) => (
                    <li key={j} className="flex items-start gap-3 text-sm text-foreground">
                      <XCircle className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "hsl(0, 84%, 60%)" }} />
                      {item}
                    </li>
                  ))}
                </ul>
        ...
        <div className="flex items-center justify-between pt-8 pb-16">
          {prevTopic ? (
            <button
              onClick={() => navigate(`/ops/training/${prevTopic.id}`)}
          ...
          {nextTopic ? (
            <button
              onClick={() => navigate(`/ops/training/${nextTopic.id}`)}
              className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
